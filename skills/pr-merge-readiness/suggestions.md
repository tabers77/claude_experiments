...
|---|---|---|---|
| `dev_env_friction` | `dev-stack-staleness` | 4 | Threshold 3/3 reached on 2026-06-12 (feat/auth-route-protection: test container bakes pyproject.toml, so branch's `no_auth_override` marker warns as unknown across all tiers). Recorded to `run_history.json:friction_log[]` per the observer's narrow exception — no SKILL.md edit (environmental). Project-side fix candidate: mount `./backend/pyproject.toml:/app/pyproject.toml:ro` in docker-compose.test.yml. **Escalation 2026-07-20** (dev vs dev_stable review, run 2026-07-20T11:51:04Z): same theme but a correctness escalation — a stale cached test image resolving unpinned `fastmcp 3.4.3`/`mcp 1.28.1` produced a **FALSE smoke FAILURE** (`test_mcp_external_host_not_misdirected` → 421) on a branch whose `pyproject.toml` pins `fastmcp==3.1.0`/`mcp==1.26.0`; smoke passed (577) only after `docker compose build --no-cache backend`. Impact upgraded from cosmetic-warning to **verdict-correctness** (risks a false RED, and — more dangerously — a false GREEN masking a real regression). Beyond the environmental mount, adds a **SKILL-side guard candidate**: when `pyproject.toml`/lockfiles are in the diff, force a fresh `--no-cache` backend build (or assert `importlib.metadata` installed versions == the pins) before trusting Phase 3/4 tiers. Note the pyproject.toml `:ro` mount alone does NOT fix this — deps are installed at image *build*, not read at runtime, so a rebuild/version-assert is required. |
...
**Status:** applied (signal 1 only; signal 2 routed to human)
**Applied at:** 2026-07-21T10:14:09Z
**Applied via:** **Signal 1 (adopted):** added `ROADMAP_STATUS_DOC` config key (defaults to `documentation/implementation_docs/implementation.md`, null → skip) and Phase 5 step 4d *Roadmap-doc status check* (informational, non-blocking) — when the diff implements a documented stream, the skill READS the matching stream section and REPORTS to Phase 7 the current status marker, any ❌/🟡 step that shipped in this diff (status drift), and any plan-vs-implementation divergence. It never edits the doc on the feature branch. Phase 8 Phase 5 audit-row evidence extended with `roadmap-doc: <reported: <stream> status=<marker>, divergences=<N> | skipped: <reason>>`. The optional `3-implementation-md-status-not-reported` FAIL tag was intentionally NOT added — step 4d is informational/non-blocking, so a mechanical FAIL counter with no blocking semantics would be inconsistent; the report is a default behavior of the step, not a gated one. **Signal 2 (NOT auto-applied — routed to repo owner):** whether the `implementation.md` status flip should be permitted/required IN the feature branch for this project's direct-ff-to-dev merge model contradicts CLAUDE.md's *Documentation Sync Rule* ("status updates happen directly on `dev` after the feature PR merges") and the `shared-pre-commit-check.md` protected-files list. Per the proposal's own DO-NOT-auto-apply instruction, this remains an open policy question for the repo owner: if the owner agrees, the change belongs in **CLAUDE.md + `shared-pre-commit-check.md` first**, and only then should the skill's Phase 3 protected-files handling be relaxed to match. Until then the skill keeps `implementation.md` protected on feature branches and recommends the dev-side update carrying the signal-1 divergence notes.
...
**Status:** applied
**Applied at:** 2026-07-21T10:14:09Z
**Applied via:** Rebuilt Phase 2 around a version-dispatched probe. Added step 2 (`git --version` → parse major.minor; `--write-tree` exists only on git ≥ 2.38, exits 129 on older git). Step 3 conflict-marker probe now branches on the parsed version: git ≥ 2.38 uses `git merge-tree --write-tree --name-only origin/<BASE_BRANCH> <head>`; git < 2.38 uses the old-form three-arg `git merge-tree $(git merge-base …) …` marker scan (explicitly noted as NON-authoritative — structurally blind to modify/delete conflicts). Per the strengthened clause, added step 4: an authoritative worktree merge probe (`git merge --no-commit --no-ff origin/<BASE_BRANCH>` → inspect `git status --porcelain` for unmerged paths → `git merge --abort`) that is **MANDATORY on every run** regardless of git version or step-3 outcome, with an explicit "step 4 wins" tie-break when the marker scan and the authoritative probe disagree. Phase 8 Phase 2 audit-row template updated to cite `git=<version>`, the dispatched probe + exit/markers, and the authoritative exit + unmerged-path list. Added two FAIL rules to Phase 8 step 2 — `2-git-version-fallback-not-automatic` (procedural, threshold=2) and `2-authoritative-merge-probe-not-run` (load-bearing, threshold=1) — and seeded both counters in `run_history.json:fail_counters`. Matching observer `review_log[]` entry (2026-06-24T12:51:55Z) mirrored applied.
...
**Status:** applied
**Applied at:** 2026-07-21T10:14:09Z
**Applied via:** Adopted the fail-safe content-signal override (the primary proposal). Added `RELEVANCE_PREDICATES.content_signal_overrides` to the config block with `security` (path substrings auth/authz/security/route/permission/access/policy + diff-body patterns `Depends(`, `HTTPException(401`, `HTTPException(403`, `is_admin`, `_enforce_`, `_can_access`, `current_user`) and `performance` (data-access path substrings + query-shape diff-body patterns) signals. Rewrote Phase 3 step 1a and Phase 5 step 1a so the override is evaluated BEFORE honoring any `skip_*` decision: a skip glob may only *trigger* a check, never *suppress* one a content signal demands — the override is one-directional (forces ON, never OFF). When the override fires AND the glob `triggers` matched zero diff paths (glob has drifted), `3-or-5-predicate-glob-stale-after-refactor` is recorded so the glob gets re-tuned. Also applied the lighter/project-specific arms: broadened `skip_security_category.triggers` to include `**/routes/**` (survives the routes.py→routes/ package split), added `**/permissions/**` + `**/policies/**`, and broadened `skip_performance_category.triggers` with the DB-query dirs (`**/repositories/**`, `**/dao/**`, `**/db/**`, `**/models/**`, `**/queries/**`) the 2026-06-09/06-10 occurrences showed were missing. Added FAIL rule `3-or-5-predicate-glob-stale-after-refactor` (procedural, threshold=2) to Phase 8 step 2 and seeded the counter in `run_history.json:fail_counters`. Matching observer `review_log[]` entry (2026-06-24T14:14:04Z) mirrored applied.
