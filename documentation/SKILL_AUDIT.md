...
| 20 | meta-sync-references | Skill | Library Maintenance | Fix stale cross-references across files |
...
| **Documentation generation** | Low | `meta-sync-references` syncs existing docs. No skill generates API docs, user guides, or architectural decision records (ADRs). | Low priority — most projects have their own doc tooling. Could extend `session-wrapup` to optionally generate changelog entries. |
